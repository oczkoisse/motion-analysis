...
class HMM:
    ...
    def __init__(self, states, alphabets, priorProbs=None, transitionProbs=None, observationProbs=None):
        '''
        Constructor
        '''
        ...
        if priorProbs is None or transitionProbs is None or observationProbs is None:
    ...
    def initializeProbs(self):
        ...
        #Transition probs
        diagonalEntry = 0.6
        offDiagonalEntry = 0.6/(len(self.states)-1)
    ...
    def train(self,observationSequenceList, epochs = 100, testSequence=None, priorLikelihood=True):
        print("Training HMM using Baum-Welch algorithm on %d sequences each of length %d"%(len(observationSequenceList),len(observationSequenceList[0])))
        LLS=[]
        if priorLikelihood:
